...
contract('GovernorERC721', function (accounts) {
  ...
  const NFTCollection = web3.utils.toBN(2);
  ...
  for (const { mode, Token } of TOKENS) {
    describe(`using ${Token._json.contractName}`, function () {
      beforeEach(async function () {
        ...
        await Promise.all([NFT0, NFT1, NFTCollection, NFT3, NFT4].map(tokenId => this.token.$_mint(owner, tokenId)));
        ...
        await this.helper.delegate({ token: this.token, to: voter2, tokenId: NFTCollection }, { from: owner });
